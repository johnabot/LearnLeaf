...
export async function sendTaskNotifications(userId, time, userEmail) {
    ...
    let pastTarget;
    let futureTarget;
    if (time === 'day after tomorrow (Daily)') {
        ...
        console.error('Invalid time (None)');
        return;
    }

    const specificTasks = tasks.filter(task => {
        const targetDate = new Date(task.dueDate);
        return targetDate >= pastTarget && targetDate <= futureTarget;
    });

    // Logic to send email notifications for specificTasks
    specificTasks.forEach(task => {
        // Send email notification for each task
        sendEmailNotification(task, userEmail); // Pass user's email address
    });
}

// Function to send email notification for a task
async function sendEmailNotification(task, userEmail) {
    const Notify = {
        to: userEmail,
        subject: `Task Reminder: ${task.assignment}`,
        text: `You have a task "${task.assignment}" due on ${task.dueDate}.\n\nDescription: ${task.description}`
    };

    try {
        // Send email
        await transporter.sendMail(Notify);
        console.log(`Email notification sent for task: ${task.assignment}`);
    } catch (error) {
        console.error(`Error sending email notification for task: ${task.assignment}`, error);
    }
}

// Call sendTaskNotifications with appropriate parameters
sendTaskNotifications(userId, time, 'user-email@example.com');

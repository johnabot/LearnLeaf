...
// Define the archiveSubject function
export async function archiveSubject(subjectId) {
    const successArchive = "Archived is successfully implemented"; 
    const failedArchive = "Error, having difficulties archiving:";
    const db = getFirestore(); 
    const subjectRef = doc(db, "subjects", subjectId);
    return updateDoc(subjectRef, { status: 'Archived' })
    .then(() => {
      console.log(successArchive);
    })
    .catch(error => {
      console.error(failedArchive, error);
    });
}
...
export async function archiveProject(projectId) {
    ...
    return updateDoc(projectRef, { status: 'Archived' })
    .then(() => {
      console.log(successArchive);
    })
    .catch(error => {

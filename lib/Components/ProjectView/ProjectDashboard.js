...
const ProjectsDashboard = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      fetchProjects(user.id).then(fetchedProjects => setProjects(fetchedProjects)).catch(error => console.error("Error fetching projects:", error));
    }
  }, [user?.id]);
  ...
  return <div className="projects-dashboard">
            ...
            <div className="projects-grid">
                {projects.map(project => <ProjectWidget key={project.id} project={project} />)}
            </div>
        <button className="fab" onClick={toggleFormVisibility}>
                +
            </button>
            {showForm && <AddProjectForm closeForm={() => setShowForm(false)} refreshProjects={refreshProjects} />}
        </div>;
};
